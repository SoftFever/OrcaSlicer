    ...
    std::vector<PerExtruderAdjustments>    &per_extruder_adjustments)
{
    ...
    auto change_extruder_set_fan = [ this, layer_id, layer_time, &new_gcode, &overhang_fan_control, &overhang_fan_threshold, &overhang_fan_speed, &overhang_fan_speed_max, &bridge_fan_speed, &internal_bridge_fan_control, &internal_bridge_fan_speed, &supp_interface_fan_control, &supp_interface_fan_speed](bool immediately_apply) {
#define EXTRUDER_CONFIG(OPT) m_config.OPT.get_at(m_current_extruder)
        ...
        if (int(layer_id) >= close_fan_the_first_x_layers) {
            ...
            if( bridge_fan_speed < 0 ) {
                bridge_fan_speed = overhang_fan_speed_max;
            }
        ...
        if (need_set_fan) {
            if (fan_speed_change_requests[CoolingLine::TYPE_OVERHANG_FAN_START]){
                ...
                if (line->overhang_degree >= overhang_fan_threshold && overhang_fan_speed_max > overhang_fan_speed) {
                    ...
                    fan_speed = std::round(fan_speed / 5.f) * 5;
                }
