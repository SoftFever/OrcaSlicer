...
void PrintObject::_slice(const std::vector<coordf_t> &layer_height_profile)
{
    ...
    std::vector<std::vector<std::pair<ExPolygon, size_t>>> segmented_regions = this->mmu_segmentation_by_painting();
...
std::vector<std::vector<std::pair<ExPolygon, size_t>>> PrintObject::mmu_segmentation_by_painting()
{
    ...
    std::vector<std::vector<PaintedLine>>                  painted_lines(this->layers().size());
    std::vector<EdgeGrid::Grid>                            edge_grids(this->layers().size());
    ...
    for (const ModelVolume *mv : this->model_object()->volumes) {
        for (const auto &params : {std::make_pair(EnforcerBlockerType::ENFORCER, 1), std::make_pair(EnforcerBlockerType::BLOCKER, 2)}) {
            ...
            for (size_t facet_idx = 0; facet_idx < custom_facets.indices.size(); ++facet_idx) {
                ...
                // Find lowest slice not below the triangle.
                auto first_layer = std::upper_bound(this->layers().begin(), this->layers().end(), float(min_z - EPSILON),
                                                    [](float z, const Layer *l1) { return z < l1->slice_z; });
                auto last_layer  = std::upper_bound(this->layers().begin(), this->layers().end(), float(max_z + EPSILON),
                                                   [](float z, const Layer *l1) { return z < l1->slice_z; });
                --last_layer;
                ...
                for (auto layer_it = first_layer; layer_it != (last_layer + 1); ++layer_it) {
                    ...
                    line_start -= this->center_offset();
                    line_end   -= this->center_offset();
