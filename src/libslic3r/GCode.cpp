...
std::string GCode::_extrude(const ExtrusionPath &path, std::string description, double speed)
{
    ...
    if (m_config.default_acceleration.value > 0) {
        ...
        } else if (m_config.sparse_infill_acceleration.value > 0 && (path.role() == erInternalInfill)) {
            acceleration = m_config.sparse_infill_acceleration.value;
        } else if (m_config.internal_solid_infill_acceleration.value > 0 && (path.role() == erSolidInfill)) {
            acceleration = m_config.internal_solid_infill_acceleration.value;
