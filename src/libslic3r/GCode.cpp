        ...
        if (custom_gcode != nullptr) {
            ...
            if (color_change || tool_change)
            {
                ...
                if (!single_filament_print && m600_extruder_before_layer >= 0 && first_extruder_id != (unsigned)m600_extruder_before_layer
                    ...
                    const std::string &color_change_script = gcodegen.config().change_filament_gcode.value;
                    if (!color_change_script.empty()) {
                        std::string script = color_change_script;
                        check_add_eol(script);
                        gcode += script;
                    } else {
