...
//            printf("flow spacing: %f,  is_external: %d, link_max_length: %lf\n", flow.spacing(), int(surface.is_external()), link_max_length);
        ...
        if (regex_search(v, std::regex("[+\\-%*@\'\"cmSODMR" + search_string + "]"))) { // template metalanguage of rotating infill
            ...
            f->angle += Geometry::deg2rad(angle); //PPS: removed archaic rotate_angle config 
        } else {
            rotate_angles.deserialize(v);
            auto rotate_angle_idx = f->layer_id % rotate_angles.size();
            f->angle += Geometry::deg2rad(rotate_angles.values[rotate_angle_idx]); //PPS: removed archaic rotate_angle config 
        }
        f->rotate_angle = 0.; //PPS: disable rotate_angle. They use only with some patterns for external alternating but another patterns unuse this parameter for same tasks
