...
void SLAPrint::Steps::rasterize()
{
    ...
    auto lvlfn = [this, &slck, &printer, increment, &dstatus, &pst] (unsigned level_id)
    {
        if(canceled()) return;
        
        PrintLayer& printlayer = m_print->m_printer_input[level_id];
    ...
    stats.used_material = (print_statistics.objects_used_material +
                           print_statistics.support_used_material) /
            1000;
