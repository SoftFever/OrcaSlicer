...
private:
    ...
    // should be executed
    std::string     extrude_entity(const ExtrusionEntity &entity, std::string description = "", double speed = -1., ExtrusionEntitiesPtr region_perimeters = ExtrusionEntitiesPtr());
    // Orca: pass the complete collection of region perimeters to the extrude loop to check whether the wipe before external loop
    // should be executed
    std::string     extrude_loop(ExtrusionLoop loop, std::string description, double speed = -1., ExtrusionEntitiesPtr region_perimeters = ExtrusionEntitiesPtr());
