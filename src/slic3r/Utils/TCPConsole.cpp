...
void TCPConsole::transmit_next_command()
{
    ...
    BOOST_LOG_TRIVIAL(debug) << boost::format("TCPConsole: transmitting '%3%' : '%4%' to %1%:%2%") % m_host_name % m_port_name % cmd.message % cmd.messageType;
...
void TCPConsole::handle_write(const boost::system::error_code &ec, std::size_t bytes, SerialMessageType messageType)
{
    BOOST_LOG_TRIVIAL(info) << boost::format("TCPConsole: Sending %2% bytes message with type %1%") % messageType % bytes;
    m_error_code = ec;
    if (ec) {
