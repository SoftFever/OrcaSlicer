...
HelioQuery::CreateSimulationResult HelioBackgroundProcess::create_simulation_step(HelioQuery::CreateGCodeResult create_gcode_res, std::unique_ptr<GUI::NotificationManager>& notification_manager)
{
    if (create_gcode_res.success) {
        ...
        HelioQuery::CreateSimulationResult create_simulation_res = HelioQuery::create_simulation(helio_api_url, helio_api_key, gcode_id, initial_room_airtemp, layer_threshold,
                                      object_proximity_airtemp);

        if (create_simulation_res.success) {
			notification_manager->push_notification((boost::format("Helio: Simulation successfully created.\n%1%") % create_simulation_res.id).str());
        } else {
