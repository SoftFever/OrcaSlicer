...
RENDER_AGAIN:
    ...
    if (m_editing_mode) {
        ...
        m_imgui->text(text);
        float slider_left = m_imgui->get_style_scaling() * (m_imgui->calc_text_size(text)).x + m_imgui->scaled(1.f);
        ...
        float sliders_left = m_imgui->get_style_scaling() * std::max(m_imgui->calc_text_size(text1).x, m_imgui->calc_text_size(text2).x) + m_imgui->scaled(1.f);
        m_imgui->text(text1);
    ...
    float slider_left = m_imgui->get_style_scaling() * std::max(m_imgui->calc_text_size(text1).x, m_imgui->calc_text_size(text2).x) + m_imgui->scaled(1.5f);
    if (m_clipping_plane_distance == 0.f)
        m_imgui->text(text1);
    else {
