...
bool GLGizmosManager::init_icon_textures()
{
    ...
    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/canvas_menu.svg", 64, 64, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_MENU, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/canvas_menu_hover.svg", 64, 64, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_MENU_HOVER, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/canvas_menu_dark.svg", 64, 64, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_MENU_DARK, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/canvas_menu_dark_hover.svg", 64, 64, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_MENU_DARK_HOVER, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/canvas_zoom.svg", 64, 64, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_ZOOM, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/canvas_zoom_hover.svg", 64, 64, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_ZOOM_HOVER, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/canvas_zoom_dark.svg", 64, 64, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_ZOOM_DARK, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/canvas_zoom_dark_hover.svg", 64, 64, texture_id))
