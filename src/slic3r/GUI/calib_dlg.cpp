...
Input_Shaping_Base_Test_Dlg::Input_Shaping_Base_Test_Dlg(wxWindow* parent, wxWindowID id, Plater* plater)
    : DPIDialog(parent, id, _L("Input shaping base test"), wxDefaultPosition, parent->FromDIP(wxSize(-1, 280)), wxDEFAULT_DIALOG_STYLE), m_plater(plater)
{
    SetBackgroundColour(*wxWHITE); // make sure background color set for dialog
    SetForegroundColour(wxColour("#363636"));
    SetFont(Label::Body_14);
    ...
    dlg_btns->GetOK()->Bind(wxEVT_BUTTON, &Input_Shaping_Base_Test_Dlg::on_start, this);
...
Input_Shaping_Base_Test_Dlg::~Input_Shaping_Base_Test_Dlg() {
    // Disconnect Events
}

void Input_Shaping_Base_Test_Dlg::on_start(wxCommandEvent& event) {
    bool read_double = false;
    ...
    m_params.mode = CalibMode::Calib_Input_shaping_base;
    
    // Set model type based on selection
    m_params.test_model = m_rbModel->GetSelection() == 0 ? 0 : 1; // 0 = Ringing Tower, 1 = Fast Tower

    m_plater->calib_input_shaping_base(m_params);
    EndModal(wxID_OK);
}

void Input_Shaping_Base_Test_Dlg::on_dpi_changed(const wxRect& suggested_rect) {
    this->Refresh();
    Fit();
}
