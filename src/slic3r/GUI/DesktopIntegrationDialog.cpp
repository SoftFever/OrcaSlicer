...
bool DesktopIntegrationDialog::is_integrated()
{
    ...
    // confirmation that GingerSlicer.desktop exists
    struct stat buffer;   
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        // Copy icon GingerSlicer.png from resources_dir()/icons to target_dir_icons/icons/
        if (contains_path_dir(target_candidates[i], "images")) {
            target_dir_icons = target_candidates[i];
            std::string icon_path = GUI::format("%1%/images/GingerSlicer.png",resources_dir());
            std::string dest_path = GUI::format("%1%/images/%2%GingerSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
    ...
    if(target_dir_icons.empty()) {
        BOOST_LOG_TRIVIAL(error) << "Copying GingerSlicer icon to icons directory failed.";
    } else 
    	// save path to icon
        app_config->set("desktop_integration_icon_slicer_path", GUI::format("%1%/images/%2%GingerSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i)
    {
        if (contains_path_dir(target_candidates[i], "applications")) {
            ...
            if (create_desktop_file(path, desktop_file)){
                BOOST_LOG_TRIVIAL(debug) << "GingerSlicer.desktop file installation success.";
                break;
            } else {
            	// write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to GingerSlicer.desktop file installation failed. failed path: " << target_candidates[i];
    ...
    app_config->set("desktop_integration_app_path", GUI::format("%1%/applications/GingerSlicer%2%.desktop", target_dir_desktop, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        ...
        if (!target_dir_icons.empty())
        {
            std::string icon_path = GUI::format("%1%/images/GingerSlicer-gcodeviewer_192px.png",resources_dir());
            std::string dest_path = GUI::format("%1%/images/%2%GingerSlicer-gcodeviewer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
            if (copy_icon(icon_path, dest_path))
                // save path to icon
        ...
        std::string desktop_file = GUI::format(
            ...
            "GenericName=3D Printing Software\n"
            "Icon=GingerSlicer-gcodeviewer%2%\n"
        ...
        std::string desktop_path = GUI::format("%1%/applications/GingerSlicerGcodeViewer%2%.desktop", target_dir_desktop, version_suffix);
        if (create_desktop_file(desktop_path, desktop_file))
            ...
            show_error(nullptr, _L("Performing desktop integration failed - could not create Gcodeviewer desktop file. GingerSlicer desktop file was probably created successfully."));
        }
    }
...
void DesktopIntegrationDialog::perform_downloader_desktop_integration(std::string url_prefix)
{
    ...
    std::string desktop_file_downloader = GUI::format(
        "[Desktop Entry]\n"
        "Name=GingerSlicer URL Protocol %1% %2%\n"
        "Exec=%3% %%u\n"
        "Terminal=false\n"
        "Type=Application\n"
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        if (contains_path_dir(target_candidates[i], "applications")) {
            target_dir_desktop = target_candidates[i];
            // Write slicer desktop file
            std::string path = GUI::format("%1%/applications/GingerSlicerURLProtocol-%2%%3%.desktop", target_dir_desktop, url_prefix, version_suffix);
            if (create_desktop_file(path, desktop_file_downloader)) {
                app_config->set("desktop_integration_URL_path", path);
                candidate_found = true;
                BOOST_LOG_TRIVIAL(debug) << "GingerSlicerURLProtocol.desktop file installation success.";
                break;
            }
            else {
                // write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to GingerSlicerURLProtocol.desktop file installation failed. failed path: " << target_candidates[i];
                target_dir_desktop.clear();
            }
        }
    }
    ...
    std::string desktop_path = GUI::format("%1%/applications/GingerSlicerURLProtocol-%2%%3%.desktop", target_dir_desktop, url_prefix, version_suffix);
    if (create_desktop_file(desktop_path, desktop_file_downloader)) {
        ...
        std::string command = GUI::format("xdg-mime default GingerSlicerURLProtocol-%1%%2%.desktop x-scheme-handler/%1%", url_prefix, version_suffix);
