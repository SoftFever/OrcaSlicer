...
_L("Upload to Printer Host with the following filename:"),
0) // Set style = 0 to avoid default creation of the "OK" button.
// All buttons will be added later in this constructor
, txt_filename(new wxTextCtrl(this, wxID_ANY))
, combo_groups(!groups.IsEmpty() ?
new wxComboBox(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, groups, wxCB_READONLY) :
nullptr)
, combo_storage(storage_names.GetCount() > 1 ?
new wxComboBox(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, storage_names, wxCB_READONLY) :
nullptr)
, post_upload_action(PrintHostPostUploadAction::None)
, m_paths(storage_paths)
, m_switch_to_device_tab(switch_to_device_tab)
...
warning_text =
new wxStaticText(this, wxID_ANY, _L("The selected bed type does not match the file. Please confirm before starting the print."),
...
fs::path ElegooPrintHostSendDialog::filename() const { return into_path(txt_filename->GetValue()); }

PrintHostPostUploadAction ElegooPrintHostSendDialog::post_action() const { return post_upload_action; }

std::string ElegooPrintHostSendDialog::group() const
{
if (combo_groups == nullptr) {
return "";
} else {
...
std::string ElegooPrintHostSendDialog::storage() const
{
if (!combo_storage)
return GUI::format("%1%", m_preselected_storage);
if (combo_storage->GetSelection() < 0 || combo_storage->GetSelection() >= int(m_paths.size()))
